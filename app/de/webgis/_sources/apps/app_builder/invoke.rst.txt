...
Neben den zwingenden URL-Parametern ``app`` und ``category`` können noch weitere optionale Parameter übergeben werden, um in der App beispielsweise auf einen bestimmten Kartenausschnitt zu springen:

* ``bbox``: Die Boundingbox, auf die gesprungen werden sollte. Es kann dafür ebenfalls noch der Parameter ``srs`` übergeben werden, um den EPSG-Code für die Boundingbox anzugeben (Standard: 4326).

* ``center`` und ``scale``: Kartenmittelpunkt (Longitude, Latitude) und Maßstab.

.. note::
   Wird die App beispielsweise aus dem Kartenviewer aufgerufen, empfiehlt es sich meistens, ``center`` und ``scale`` anstelle von ``bbox`` zu verwenden. Da die Größe des Kartenfensters nicht immer gleich ist, wird die Anwendung mit ``bbox`` nicht mit dem gleichen Maßstab geöffnet.
...
Wie schon beschrieben, kann man eine App auch über einen Link mit Übergabe des aktuellen Kartenausschnitts aufrufen. Dadurch kann eine App sehr einfach als *benutzerdefiniertes Werkzeug* in den Kartenviewer eingebunden werden.

Benutzerdefinierte Werkzeuge werden in der ``custom.js``-Datei definiert (`siehe <./../KartenViewer/CustomJS/benutzerdefmarker.html#benutzerdefinierte-werkzeuge>`__).

Die Definition des Werkzeugs sieht etwa folgendermaßen aus:
