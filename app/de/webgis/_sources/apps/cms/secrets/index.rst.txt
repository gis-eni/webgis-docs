...
Unter ``Placeholder`` wird angezeigt, wie dieses Secret verwendet werden kann. Das Secret kann überall im CMS als Teil 
eines *ConnectionString* oder als Passwort eingefügt werden:

.. image:: img/secrets5.png

In jedem Dialog wird oben ein Symbol angezeigt, über das man die für das CMS vorhandenen *Secret-Platzhalter* anzeigen 
kann. Klickt man auf einen Platzhalter, wird
dieser in die Zwischenablage kopiert:

.. image:: img/secrets7.png

Für unterschiedliche Umgebungen (*Environments*) können unterschiedliche *Secret-Werte* angelegt werden. Ist für eine 
Umgebung kein Wert angegeben, wird der Wert aus ``Default`` verwendet. Die Umgebung für ein Deployment kann von 
Systemadministratoren in der ``cms.config`` eingestellt werden (Knoten ``deployment`` unter ``environment``).

.. note:: 
   Beim Veröffentlichen eines CMS wird die verwendete Umgebung angezeigt:
