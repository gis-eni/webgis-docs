...
This section covers further concepts of the drawing (map markup) tool:
...
.. note::
   ...
   them to delete "old" drawings from time to time, since the map markup tool should actually only be used to create temporary
   drawings (see the description "good use cases for map markup").
...
can only be interpreted again by the map markup tool when uploaded later.
...
*GPX* and *GeoJSON* (map markup projects that were previously downloaded) files can be uploaded.
