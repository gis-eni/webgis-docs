...
Depending on permissions, it is possible to take the geometry for map markup elements from an existing query/selection.
...
.. note::
   The query results must appear **selected** on the map (with a *cyan* background). Otherwise, they will not be recognized by the *drawing (map markup)* tool.
   Query results are selected when the corresponding option has been set in the result list:

   .. image:: img/selection1.png

Switching to the *drawing (map markup)* tool allows you to transfer these objects. First, note the geometry type (point, line, polygon) of the selected query results.
...
Here you can select an attribute from the query results, whose values should be used as a description when inserting the objects into the map markup. This makes it easier later to identify the graphic elements
...
+-----------------------------------------------------+----------------------------------------------------------------+
| **Object Geometry**                                 | **Drawing (Map Markup) Geometry**                              |
+-----------------------------------------------------+----------------------------------------------------------------+
| Points                                              | * Symbols                                                      |
|                                                     | * Text                                                         |
+-----------------------------------------------------+----------------------------------------------------------------+
