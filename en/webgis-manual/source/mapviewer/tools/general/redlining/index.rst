...
Drawing (redlining) is a very powerful map tool and can be used for various applications.
The basic idea of redlining is to extend the map with drawing elements (lines, polygons, text, symbols)
and to mark locations for a printout, to send, or otherwise make available.

Redlining should only be used to create short-lived drawings that also don't contain too many objects.
The drawings can be saved, but do not need to be kept forever on the server by the map viewer operator.

There is also the option to download the redlining and save it yourself. However, it should also
